...
export default function PostCard({ post }: PostCardProps) {
  return (
    <Link href={`/blog/${post.slug}`} className="block group h-full">
      <article className="h-full bg-[#1a1c21] border border-white/5 border-l-[4px] border-l-[#d4ff33] rounded-2xl p-6 transition-all duration-300 hover:bg-white/5 hover:transform hover:-translate-y-1 hover:shadow-2xl flex flex-col">
        ...
        <h3 className="text-xl font-bold text-white mb-3 group-hover:text-[#d4ff33] transition-colors leading-tight line-clamp-2">
          {post.title}
        </h3>
