    ...
    try {
      ...
      for (let j = 0; j < numCols; j++) {
        ...
        for (let i = 0; i < rows.length; i++) {
          const text = sanitizeText(String(rows[i][j] || ''));
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        const cellLines = rows[i].map((cell, j) => 
    ...
    <div className="w-full max-w-5xl mx-auto">
      <div className="bg-[#1a1c21] border border-gray-800 rounded-[32px] overflow-hidden shadow-2xl">
        <div className="p-8 border-b border-gray-800 flex items-center justify-between bg-white/5">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-bold text-white">{tt('excel-to-pdf')}</h3>
              <p className="text-gray-500 text-sm">{tt('excel-to-pdf-desc')}</p>
            </div>
          </div>
          {excelFile && (
            <button 
        ...
        <div className="p-8">
          {!excelFile ? (
            ...
            >
              ...
              <p className="text-gray-500 text-sm mb-8">{tt('excel-to-pdf-step1')}</p>
                  ...
                  {excelFile.status === 'completed' ? (
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-2 text-green-400 font-bold text-sm">
                        ...
                        {tt('excel-to-pdf-status-completed')}
                      </div>
                      <button 
                        onClick={downloadPdf}
                      ...
                      >
                        <Download size={18} /> {tt('excel-to-pdf-download')}
                      </button>
                    </div>
                  ) : excelFile.status === 'error' ? (
                    <div className="flex items-center gap-2 text-red-400 font-bold text-sm">
                      <AlertCircle size={20} />
                      {tt('excel-to-pdf-status-error')}
                    </div>
                  ) : (
                    <button 
                    ...
                    >
                      {isProcessing ? (
                        <><Loader2 size={18} className="animate-spin" /> {tt('excel-to-pdf-status-processing')}</>
                      ) : (
                        <><RefreshCw size={18} /> {tt('excel-to-pdf-apply')}</>
                      )}
                    </button>
                  )}
                </div>
      ...
      <div className="mt-20 grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8">
          ...
          <h4 className="text-white font-bold mb-4">{tt('excel-to-pdf-f1-title')}</h4>
          <p className="text-gray-500 text-sm leading-relaxed">
            {tt('excel-to-pdf-f1-desc')}
          </p>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8">
          ...
          <h4 className="text-white font-bold mb-4">{tt('excel-to-pdf-f2-title')}</h4>
          <p className="text-gray-500 text-sm leading-relaxed">
            {tt('excel-to-pdf-f2-desc')}
          </p>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8">
          ...
          <h4 className="text-white font-bold mb-4">{tt('excel-to-pdf-f3-title')}</h4>
          <p className="text-gray-500 text-sm leading-relaxed">
            {tt('excel-to-pdf-f3-desc')}
          </p>
        </div>
      </div>
    </div>
