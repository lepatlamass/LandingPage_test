...
export default function CsvToPdfTool() {
  ...
  const { guardedBlobDownload, modalState, closeModal, onLoginSuccess } = useDownloadGate('csv-to-pdf');
  ...
  const downloadPdf = () => {
    ...
    guardedBlobDownload(csvFile.resultPdf, `${csvFile.file.name.split('.')[0]}.pdf`);
  };
        ...
        <div className="p-6 md:p-8 border-b border-zinc-300 dark:border-gray-800 flex flex-col md:flex-row items-start md:items-center justify-between bg-black/5 dark:bg-white/5 gap-6 md:gap-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 text-sm">Convert structured CSV data into professional PDF tables</p>
            </div>
          </div>
          {csvFile && (
            <button 
        ...
        <div className="p-8">
          {!csvFile ? (
            ...
            >
              ...
              <p className="text-gray-600 dark:text-gray-400 text-sm mb-8">Select a CSV file to convert</p>
