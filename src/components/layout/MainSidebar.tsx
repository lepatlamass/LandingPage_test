...
export default function MainSidebar({ activeTool, onToolSelect }: MainSidebarProps) {
  ...
  return (
    <aside className="w-64 border-r border-gray-200 dark:border-gray-800 flex flex-col shrink-0 bg-gray-50 dark:bg-[#0f1115] transition-colors">
      <div className="p-6 flex items-center gap-2">
        <div className="w-8 h-8 bg-[#d4ff33] rounded-lg flex items-center justify-center text-black font-bold text-xl">
          R
        </div>
        <span className="text-gray-900 dark:text-white font-bold text-xl tracking-tight">Refinedocs</span>
      </div>

      <div className="px-4 mb-4">
        <h2 className="text-xs font-semibold text-black dark:text-gray-500 uppercase tracking-wider px-2 mb-4">Tools</h2>
              ...
              <div key={tool.id} className="space-y-1">
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 group ${
                    isStrictlyActive 
                      ? 'bg-gray-200/50 text-gray-900 dark:bg-white/10 dark:text-white' 
                      : isExpanded && tool.children
                        ? 'text-gray-900 dark:text-white'
                        : `text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-200 ${tool.hover}`
                  }`}
                >
                  <div className={`p-1.5 rounded-lg bg-gray-200 group-hover:bg-gray-300 dark:bg-gray-800 dark:group-hover:bg-gray-700 transition-colors ${tool.color}`}>
                    <tool.icon size={18} />
                  </div>
